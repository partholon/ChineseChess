...
protected:
	...
	virtual bool open_port()
	{
		...
		{
			//modified by liuzhe at 2014/3/26
			//加入日志
			CString strLog;
			LPVOID lpMsgBuf;
			LPVOID lpDisplayBuf;
			...
			FormatMessage(
				...
				(LPTSTR) &lpMsgBuf,
				0, NULL );
			strLog.Format("COM open error %s", lpMsgBuf);
			LOG(strLog);
			LocalFree(lpMsgBuf);
			return false;
		}
...
protected:
	...
	virtual bool open_port()
	{
		...
		//modify by partholon 2016/6/22
		//解决串口号大于10无法打开的问题
		if (_port > 9) {
			strcpy(com_str, "\\\\.\\COM");
			ltoa(_port, com_str + 7, 10);
			_com_handle = CreateFile(
				com_str,
				GENERIC_READ | GENERIC_WRITE,
				0,
...
protected:
	...
	virtual bool open_port()
	{
		if(_asyn_com::open_port())
		{
			...
			//modified by liuzhe at 2014/3/26
			//创建线程后先暂停线程
			_thread_handle = CreateThread(NULL, 0, com_thread, this, CREATE_SUSPENDED, &id); //辅助线程
